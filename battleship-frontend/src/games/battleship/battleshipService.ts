import { Client as BattleshipClient, type Game } from './bindings';
...
/**
 * Service for interacting with the Battleship game contract
 */
export class BattleshipService {
  private baseClient: BattleshipClient;
  private contractId: string;

  constructor(contractId: string) {
    this.contractId = contractId;
    // Base client for read-only operations
    this.baseClient = new BattleshipClient({
  ...
  private createSigningClient(
    ...
    return new BattleshipClient(options);
  }
  ...
  async prepareStartGame(
    ...
    const buildClient = new BattleshipClient({
      contractId: this.contractId,
      networkPassphrase: NETWORK_PASSPHRASE,
      rpcUrl: RPC_URL,
  ...
  async commitBoard(
    ...
    cellCommitments: Buffer[],
    shipCells: number,
    boardProofHash: Buffer | undefined,
    boardProofSignature: Buffer | undefined,
    signer: Pick<contract.ClientOptions, 'signTransaction' | 'signAuthEntry'>,
    authTtlMinutes?: number
  ) {
    ...
    const tx = await client.commit_board(
      {
        ...
        cell_commitments: cellCommitments,
        ship_cells: shipCells,
        board_proof_hash: boardProofHash,
        board_proof_signature: boardProofSignature,
      },
      DEFAULT_METHOD_OPTIONS
    );
  ...
  async commitBoardZk(
    ...
    const client = this.createSigningClient(playerAddress, signer);
    const tx = await client.commit_board_zk(
      {
        session_id: sessionId,
        player: playerAddress,
        cell_commitments: cellCommitments,
        ship_cells: shipCells,
        zk_board_proof: zkBoardProof,
      },
      DEFAULT_METHOD_OPTIONS
    );
  ...
  async depositStake(
    ...
    const client = this.createSigningClient(playerAddress, signer);
    const tx = await client.deposit_stake(
      {
        session_id: sessionId,
        player: playerAddress,
      },
  ...
  async getBetToken(): Promise<string | undefined> {
    try {
      const tx = await this.baseClient.get_bet_token();
      const result = await tx.simulate();
  ...
  async getVerifier(): Promise<Buffer | undefined> {
    try {
  ...
  async getZkVerifierContract(): Promise<string | undefined> {
    try {
      const tx = await this.baseClient.get_zk_verifier();
      const result = await tx.simulate();
      return result.result;
    } catch {
      return undefined;
...
// Example: const battleshipService = new BattleshipService(contractId);
