...
import {
  ...
  disconnect as disconnectWalletKit,
  getContractSigner as getWalletKitSigner,
  getPublicKey as getWalletKitPublicKey,
  getSelectedWallet,
} from '../services/stellarWalletKitService';
...
export function useWallet() {
  ...
  const connectWallet = useCallback(async () => {
    try {
      ...
      const address = await getWalletKitPublicKey();
      const walletId = getSelectedWallet();

      if (!address || !walletId) {
        throw new Error('Wallet connected but account address is unavailable.');
      }
  ...
  const getContractSigner = useCallback((): ContractSigner => {
    ...
    if (walletType === 'wallet') {
      return getWalletKitSigner(publicKey);
    } else {
